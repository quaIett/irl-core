...
public final class VlGlobalsBuffer
{
    ...
    private static final int CAPACITY = 48;     // 3 × vec4 (std140)
     ...
     *  visibly every wrap. Steps/strides are clamped to the same sane
     *  ranges the injected GLSL clamps to. */
    public static void set(float intensity, float maxDist, float tipBoost, float tipRadius, float noiseAmount, float noiseScale, float noiseSpeed, int stepMax, int shadowStride, int noiseStride, int flags)
    {
